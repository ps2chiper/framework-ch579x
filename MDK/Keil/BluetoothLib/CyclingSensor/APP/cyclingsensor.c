...
uint16 CyclingSensor_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
    // Start the Device
    VOID GAPRole_PeripheralStartDevice( &cyclingPeripheralCB );
  ...
  if ( events & CSC_CONN_PARAM_UPDATE_EVT )
  {
    ...
    GAPRole_PeripheralSendUpdateParam( DEFAULT_DESIRED_MIN_CONN_INTERVAL, DEFAULT_DESIRED_MAX_CONN_INTERVAL,
  ...
  if ( events & CSC_NEGLECT_TIMEOUT_EVT )
  {
    ...
    GAPRole_PeripheralTerminateLink();
